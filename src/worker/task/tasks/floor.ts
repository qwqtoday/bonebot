...
export default {
	...
	run(bot) {
		...
		// set area
		const area: Cuboid = info.area;

		// values that can change every tick
		const shouldX = area.point1.x + (info.row * 3 + 1.5);
		const startZ = info.row % 2 == 0 ? area.point1.z + 1 : area.point2.z - 1;
		const goalZ = info.row % 2 == 0 ? area.point2.z - 1 : area.point1.z + 1;
