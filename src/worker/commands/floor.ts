...
export default {
	...
	async execute(context, bot) {
		const blockName = context.args[0];
		const point_a = context.args[1];
		const point_b = context.args[2];
		...
		const [ax, ay, az] = point_a.split(',').map((n) => parseInt(n));
		const [bx, by, bz] = point_b.split(',').map((n) => parseInt(n));
