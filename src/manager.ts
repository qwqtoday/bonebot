...
export async function createWorkerManager(config: Config, db: NodePgDatabase): Promise<WorkerManager> {
	...
	const all_workers = await db.select().from(workers);
	all_workers
	...
	async function loadWorker(name: string) {
		const res = await db.select().from(workers).where(eq(workers.nickname, name));
		const workerData = res[0];
	...
	function getWorker(name: string): Worker {
		...
		if (worker === undefined) {throw new ClientManagerError('No worker with that specified name.', 404);}
	...
	function startWorker(name: string) {
		...
		if (client.state != 'STOPPED') {throw new ClientManagerError('Client is not stopped.', 409);}
	...
	function stopWorker(name: string) {
		const client = getWorker(name);
		if (client.state == 'STOPPED') {throw new ClientManagerError('Client is already stopped.', 409);}
	...
	function getWorkerInfo(name: string): WorkerInfo {
		...
		if (worker.state != 'STARTED') {throw new ClientManagerError('Client is not started.', 409);}
